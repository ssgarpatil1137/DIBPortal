...
IF COL_LENGTH('dbo.SpendItems','LineId') IS NULL ALTER TABLE dbo.SpendItems ADD LineId nvarchar(100) NULL;
GO
...
CREATE OR ALTER PROCEDURE dbo.sp_SaveSpendItem
 ...
 @Description nvarchar(500)=NULL,@YearlyRecurrence int=NULL,@LineId nvarchar(100)=NULL
AS
BEGIN
 SET NOCOUNT ON;
 ...
 IF @Id IS NULL
 BEGIN
  INSERT dbo.SpendItems(PetId,LineId,Department,Head,Topic,Vendor,Description,CostType,UnitType,Units,UnitPrice,Currency,ForeignAmount,AedAmount,ContingencyPercent,GlNumber,YearlyRecurrence)
  VALUES(@Pet,@LineId,@Department,@Head,@Topic,@Vendor,@Description,@CostType,@UnitType,@Units,@UnitPrice,@Currency,@Foreign,@Aed,@Contingency,@Gl,@YearlyRecurrence);
  SET @Id=SCOPE_IDENTITY();
 END
 ELSE
  UPDATE dbo.SpendItems SET LineId=@LineId,Department=@Department,Head=@Head,Topic=@Topic,Vendor=@Vendor,Description=@Description,CostType=@CostType,UnitType=@UnitType,Units=@Units,UnitPrice=@UnitPrice,Currency=@Currency,ForeignAmount=@Foreign,AedAmount=@Aed,ContingencyPercent=@Contingency,GlNumber=@Gl,YearlyRecurrence=@YearlyRecurrence WHERE SpendItemId=@Id AND PetId=@Pet;
