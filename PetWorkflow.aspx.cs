...
namespace DFM_BPM.Forms
{
    public partial class PetWorkflow : Page
    {
        ...
        protected bool CanEditSpendLines
        {
            get { return IsEditable && AuthHelper.IsRequestor; }
        }
        ...
        /// <summary>Delete PET Request button/action is only available while the request is still Draft or
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    // Arrived from Project Registration's "Create PET Request" action — pre-select the project.
        ...
        {
            bool canEditHeader = CanEditSpendLines;
            ddlProject.Enabled      = canEditHeader && !IsProjectLocked;
            ddlType.Enabled         = canEditHeader;
            ddlBudgetSource.Enabled = canEditHeader;
            ddlReviewer.Enabled     = canEditHeader;
            ddlApprover.Enabled     = canEditHeader;
            txtTitle.Enabled        = canEditHeader;
            txtVertical.Enabled     = canEditHeader;
            txtApplications.Enabled = canEditHeader;
        }

        private void ApplyInlineLineState()
        {
            pnlInlineSpendLines.Visible = CurrentPetFormId > 0 || CanEditSpendLines;
            pnlInlineLineEditor.Visible = CanEditSpendLines;
            btnCancelLine.Visible = CanEditSpendLines && !string.IsNullOrEmpty(hfEditLineId.Value);
            btnSaveLine.Text = string.IsNullOrEmpty(hfEditLineId.Value) ? "Add Spend Line" : "Update PET Line";
        }
        ...
        private void LoadForm(int id)
        {
            ...
            txtVertical.Text = f.Table.Columns.Contains("Vertical") && f["Vertical"] != DBNull.Value ? f["Vertical"].ToString() : "";
            txtApplications.Text = f.Table.Columns.Contains("Applications") && f["Applications"] != DBNull.Value ? f["Applications"].ToString() : "";
            ...
            pnlInlineSpendLines.Visible = true;
            BindLines(id);
        ...
        /// <summary>The PET cost/invoice summary row was removed from the request header budget panels.</summary>
        ...
        private void ResequenceStagedLines(DataTable dt)
        {
        ...
        private void PopulateLineEntry(DataRow r)
        {
            hfEditLineId.Value = r["LineID"].ToString();
            btnSaveLine.Text = "Update PET Line";
            ...
            SetDdl(ddlLineGL,       r["GLNumber"]      == DBNull.Value ? "" : r["GLNumber"].ToString());
        ...
        {
            if (!AuthHelper.IsRequestor) { ShowMsg("Only Requestor can save PET Requests."); return; }
            ...
            if (string.IsNullOrEmpty(projectId))
            {
                ShowMsg("A Demand ID# is required. Register it first via the Project Registration page.");
                return;
            }
            ...
            string vertical = txtVertical.Text.Trim();
            string applications = txtApplications.Text.Trim();

            if (string.IsNullOrEmpty(approverVal)) { ShowMsg("Approver is required before saving the PET Request."); return; }
            ...
            if (CurrentPetFormId == 0)
            {
                ...
                Session["PetNextStep"] = "PET Request saved. Next: attach supporting documents if needed, then submit it for approval.";
            ...
            ShowNextStep("PET Request saved. Next: review or add line items, attach documents if needed, then submit it for approval.");
            pnlLines.Visible = true;
            pnlLinesGrid.Visible = true;
            pnlInlineSpendLines.Visible = true;
            ShowBudgetAmounts(ddlType.SelectedValue, ddlBudgetSource.SelectedValue);
        ...
        protected void btnCancelPet_Click(object sender, EventArgs e)
        {
            ViewState[StagedLinesKey] = null;
            ViewState[NextStagedLineIdKey] = null;
        ...
        {
            if (!CanEditSpendLines) { ShowMsg("Only Requestor can add or update PET lines."); return; }
            ClearLineEntry();
            ApplyInlineLineState();
        }
        ...
        {
            if (!CanEditSpendLines) { ShowMsg("Only Requestor can add or update PET lines."); return; }
            string expHead = ddlExpHead.SelectedValue;
            string topic = txtLineTopic.Text.Trim();
            string vendor = ddlLineVendor.SelectedValue;
            string costType = ddlLineCostType.SelectedValue;
            string currency = ddlLineCcy.SelectedValue;
            string glNumber = ddlLineGL.SelectedValue;
            ...
            if (string.IsNullOrEmpty(topic)) { ShowMsg("Topic / Item is required for the PET line."); return; }
            if (string.IsNullOrEmpty(vendor)) { ShowMsg("Vendor is required for the PET line."); return; }
            if (string.IsNullOrEmpty(costType)) { ShowMsg("Cost Type is required for the PET line."); return; }
            if (units <= 0) { ShowMsg("Units must be greater than zero."); return; }
            if (price <= 0) { ShowMsg("Unit Price must be greater than zero."); return; }

            int editId;
            if (CurrentPetFormId == 0)
            {
                DataTable staged = GetStagedLines();
                DataRow row = null;
                if (int.TryParse(hfEditLineId.Value, out editId))
                {
                    DataRow[] rows = staged.Select("LineID=" + editId.ToString());
                ...
                {
                    row = staged.NewRow();
                    row["LineID"] = GetNextStagedLineId();
                    staged.Rows.Add(row);
                }
                ...
                row["Description"] = txtLineDesc.Text.Trim();
                ...
                row["AmtFCY"] = fcy;
                row["AmtLCY"] = lcy;
                row["ContingencyPct"] = cont;
                row["FinalAmtLCY"] = final;
                row["GLNumber"] = glNumber ?? "";
                row["Comments"] = "";
                ResequenceStagedLines(staged);
                ViewState[StagedLinesKey] = staged;
                ClearLineEntry();
                BindLines(0);
                ShowNextStep("PET line staged. Next: add another line if required, or click Save Project & PET.");
                return;
            }
            ...
            BindLines(CurrentPetFormId);
            ClearLineEntry();
            ShowNextStep("Line item saved. Next: add another line item if required, or go to Approval and submit the PET Request.");
        }
        ...
        {
            if (!CanEditSpendLines) { ShowMsg("Only Requestor can add, update, or delete PET lines."); return; }
            ...
            if (e.CommandName == "DelLine")
            {
                ...
                {
                    DataTable staged = GetStagedLines();
                    DataRow[] rows = staged.Select("LineID=" + lineId.ToString());
                    if (rows.Length > 0) staged.Rows.Remove(rows[0]);
                    ResequenceStagedLines(staged);
                    ViewState[StagedLinesKey] = staged;
                    ClearLineEntry();
                    BindLines(0);
            ...
            else if (e.CommandName == "EditLine")
            {
                int lineId = Convert.ToInt32(e.CommandArgument);
                DataRow r = null;
                if (CurrentPetFormId == 0)
                {
                    DataRow[] rows = GetStagedLines().Select("LineID=" + lineId.ToString());
                    if (rows.Length > 0) r = rows[0];
                }
                else
                {
                    r = Db.QueryRow("SELECT * FROM dbo.PetLineItem WHERE LineID=@id", Db.P("@id", lineId));
                }
                if (r == null) return;
        ...
        {
            if (!CanEditSpendLines) { ShowMsg("Only Requestor can upload attachments for PET Requests."); return; }
        ...
        {
            if (e.CommandName == "DelAttach" && CanEditSpendLines)
            {
        ...
        {
            if (!CanEditSpendLines) { ShowMsg("Only Requestor can submit PET Requests."); return; }
            if (CurrentPetFormId == 0) { ShowMsg("Save the form header first."); return; }
            DataRow f = WorkflowDAL.GetPetForm(CurrentPetFormId);
            if (f == null) { ShowMsg("PET Request not found."); return; }
            ...
            Session["PetNextStep"] = f["ReviewerUsername"] == DBNull.Value || string.IsNullOrEmpty(Convert.ToString(f["ReviewerUsername"]))
                ? "PET Request submitted successfully. Next: the approver will review it."
                : "PET Request submitted successfully. Next: the reviewer will review it and route it to the approver.";
        ...
        protected void btnApprove_Click(object sender, EventArgs e)
        {
            ...
            if (status == "PendingReview")
            {
                ...
                Session["PetNextStep"] = "Review submitted successfully. Next: the approver will approve or send back this PET Request.";
            }
            else if (status == "PendingApproval")
            {
                ...
                Session["PetNextStep"] = "PET Request approved successfully. Next: the requestor can manage Budget and Invoice details.";
            }
            Response.Redirect("~/Forms/PetWorkflow.aspx?id=" + CurrentPetFormId);
        }
        ...
        protected void btnReject_Click(object sender, EventArgs e)
        {
            ...
            Session["PetNextStep"] = "PET Request rejected successfully. Next: the requestor can review the comments and create a revised request if needed.";
            Response.Redirect("~/Forms/PetWorkflow.aspx?id=" + CurrentPetFormId);
        }
        ...
        protected void btnSendBack_Click(object sender, EventArgs e)
        {
            ...
            Session["PetNextStep"] = "PET Request sent back successfully. Next: the requestor should update it and submit again.";
            Response.Redirect("~/Forms/PetWorkflow.aspx?id=" + CurrentPetFormId);
        }
        ...
        protected void btnSizingSave_Click(object sender, EventArgs e)
        {
            ...
            LoadSizingForProject(projectId);
            ShowNextStep("Sizing saved. Size: " + sizeResult + "  (Weighted score: " + weighted.ToString("F4") + "). Next: continue the PET Request details or submit for approval when line items are ready.");
        }
