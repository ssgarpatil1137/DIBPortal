...
namespace DFM_BPM.App_Code.DAL
{
    ...
    public static class WorkflowDAL
    {
        ...
        public static int CreatePetForm(string projectId, string capexOpexType, string budgetSourceId,
                                        ...
                                        string vertical = null, string applications = null)
        {
            return Convert.ToInt32(Db.Scalar(@"
                ...
                VALUES
                    (@pid, @cot, @bsid, @ti, @de, @rev, @app, 'Draft', @cb, @nj, @pn, @ver, @apps)",
                ...
                Db.P("@ver", string.IsNullOrEmpty(vertical) ? (object)DBNull.Value : vertical),
                Db.P("@apps", string.IsNullOrEmpty(applications) ? (object)DBNull.Value : applications)));
        }

        public static int CreatePetFormWithLines(string projectId, string capexOpexType, string budgetSourceId,
                                                string title, string description, string reviewerUsername,
                                                string approverUsername, string createdBy,
                                                bool isNonJiraProject, string projectName,
                                                string vertical, string applications, DataTable lines)
        {
                ...
                using (SqlTransaction tx = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(@"
                            ...
                            OUTPUT INSERTED.PetFormID
                            VALUES
                                (@pid, @cot, @bsid, @ti, @de, @rev, @app, 'Draft', @cb, @nj, @pn, @ver, @apps)", conn, tx))
                        {
                            cmd.CommandTimeout = 150;
                            cmd.Parameters.AddRange(new[] {
                                Db.P("@pid", projectId),
                                Db.P("@cot", capexOpexType),
                                Db.P("@bsid", string.IsNullOrEmpty(budgetSourceId) ? (object)DBNull.Value : budgetSourceId),
                                Db.P("@ti", title ?? ""),
                                Db.P("@de", description ?? ""),
                                Db.P("@rev", string.IsNullOrEmpty(reviewerUsername) ? (object)DBNull.Value : reviewerUsername),
                                Db.P("@app", string.IsNullOrEmpty(approverUsername) ? (object)DBNull.Value : approverUsername),
                                Db.P("@cb", createdBy),
                            ...
                            petFormId = Convert.ToInt32(cmd.ExecuteScalar());
                        }
                            ...
                            foreach (DataRow row in lines.Rows)
                            {
                                using (SqlCommand cmd = new SqlCommand(@"
                                    ...
                                    VALUES (@fid, @sn, @dep, @eh, @to, @vn, @de, @ct,
                                            @un, @up, @cu, @af, @al, @cp, @fl, @gl, @co, @cb)", conn, tx))
                                {
                                    cmd.CommandTimeout = 150;
                                    cmd.Parameters.AddRange(new[] {
                                        Db.P("@fid", petFormId),
                                        Db.P("@sn", row["SerialNo"]),
                                        Db.P("@dep", row["Department"]),
                                        Db.P("@eh", row["ExpHead"]),
                                        Db.P("@to", row["Topic"]),
                                        Db.P("@vn", row["VendorName"]),
                                        Db.P("@de", row["Description"]),
                                        Db.P("@ct", row["CostType"]),
                                        Db.P("@un", row["Units"]),
                                        Db.P("@up", row["UnitPrice"]),
                                        Db.P("@cu", row["BaseCurrency"]),
                                        Db.P("@af", row["AmtFCY"]),
                                        Db.P("@al", row["AmtLCY"]),
                                        Db.P("@cp", row["ContingencyPct"]),
                                        Db.P("@fl", row["FinalAmtLCY"]),
                                        Db.P("@gl", string.IsNullOrEmpty(Convert.ToString(row["GLNumber"])) ? (object)DBNull.Value : row["GLNumber"]),
                                        Db.P("@co", row["Comments"]),
                                        Db.P("@cb", createdBy)
                                    });
                                    cmd.ExecuteNonQuery();
                                }
                                         ...
                                         string vertical = null, string applications = null)
        {
            Db.Exec(@"UPDATE dbo.PetForm SET ProjectID=@pid, CapexOpexType=@cot, BudgetSourceID=@bsid,
                      ...
                      IsNonJiraProject=@nj, ProjectName=@pn, Vertical=@ver, Applications=@apps,
        ...
        /// <summary>Status values for which a PET Request may still be deleted (soft delete) — once
